...
func TestBoringSSLFactory_RetransmitsLostFirstFlight(t *testing.T) {
	for _, hook := range []bool{false, true} {
		name := "WithoutHook"
		if hook {
			name = "WholeFlightFallback"
		}
		t.Run(name, func(t *testing.T) { testBoringSSLRetransmitsLostFirstFlight(t, hook) })
	}
}

func testBoringSSLRetransmitsLostFirstFlight(t *testing.T, hook bool) {
	...
	if hook {
		pair.server.(interface {
			SetOutboundHandshakePacketInterceptor(func([][]byte, net.Addr) bool)
		}).SetOutboundHandshakePacketInterceptor(func([][]byte, net.Addr) bool { return false })
	}
